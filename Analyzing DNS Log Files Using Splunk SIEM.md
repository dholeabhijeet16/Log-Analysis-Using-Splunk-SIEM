...
- Obtain  DNS log file in a suitable format (e.g., text files).
...
- Choose the appropriate source type for DNS logs.
...
```
index=* sourcetype=dns_logs | regex _raw="(?i)\b(dns|domain|query|response|port 53)\b"
```
...
```
index=_* OR index=* sourcetype=dns_logs  | stats count by fqdn
```

### 4. Find the top DNS sources
- Use the top command to count the occurrences of each query type:   
```
index=* sourcetype=dns_logs | top fqdn, src_ip
```
